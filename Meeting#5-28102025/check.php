...
while (true) {
    ...
    //function auswahlOutput
    $aktion = auswahlOutput();

    // 5. LOGIK-VERZWEIGUNG

    // --- AKTION 1: Einzelnes Produkt prüfen ---
    if ($aktion == 1) {
        bestandPruefen($lagerbestand);
...
function auswahlOutput(): string // mixed - wir wiessen nicht was wir zurückliefern aber nicht leer Wert
{
    echo "Deine Auswahl: ";
...
function bestandPruefen(array $datenbank)
{
    echo STYLE_BOLD . "--- Einzelproduktprüfung ---\n" . COLOR_RESET;

    $skuListe = array_keys($datenbank);

    foreach ($skuListe as $index => $sku) {
        $produktName = $datenbank[$sku]['name'];
