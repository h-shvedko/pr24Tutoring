...
class Alg_WC_Checkout_Files_Upload_Main {
	...
	function alg_ajax_file_upload() {
		...
		if ( isset( $_FILES['file'] ) && '' != $_FILES['file']['tmp_name'] && isset( $_POST['file_uploader'] ) ) {
			...
			// validate min/max files
			$existing_files = get_post_meta( $order_id, '_alg_checkout_files_upload_' . $file_uploader, true );
			if ( ! $existing_files ) {
				$existing_files = array();
			}
			$validated_min_max = $this->validate_files_min_max(
				$file_uploader,
				array_merge(
					$existing_files,
					array( $_FILES['file'] )
			...
			if ( $validated_min_max !== true ) {
				// Error occurred
				// However we are only concerned with exceeding the maximum in this case, since this
				// function is called once per upload, we may still meet the minimum on a future call.
				if ( $validated_min_max['code'] === 'fail_max_files' ) {
					echo json_encode( array(
						'result'     => 0,
						'message'    => $validated_min_max['message'],
						'error_code' => $validated_min_max['code'],
					) );
					die();
				}
			}
	 ...
	 */
	public function validate_files_min_max( $file_uploader, $files ) {
		$result = true;
		$min_files = intval( get_option( 'wpwham_checkout_files_upload_min_files_' . $file_uploader ) );
		$max_files = intval( get_option( 'wpwham_checkout_files_upload_max_files_' . $file_uploader ) );
		if ( $min_files || $max_files ) {
			if ( count( $files ) > $max_files ) {
				$result = array(
					'code'    => 'fail_max_files',
					'message' => sprintf(
						( get_option( 'wpwham_checkout_files_upload_notice_too_many_' . $file_uploader ) > '' ?
							get_option( 'wpwham_checkout_files_upload_notice_too_many_' . $file_uploader )
							: __( 'Too many files uploaded. Maximum %s file(s) are allowed.', 'checkout-files-upload-woocommerce' ) ),
						$max_files 
					),
				);
			} elseif ( count( $files ) < $min_files ) {
				$result = array(
					'code'    => 'fail_min_files',
					'message' => sprintf(
						( get_option( 'wpwham_checkout_files_upload_notice_too_few_' . $file_uploader ) > '' ?
							get_option( 'wpwham_checkout_files_upload_notice_too_few_' . $file_uploader )
							: __( 'Too few files uploaded. Minimum %s file(s) are required.', 'checkout-files-upload-woocommerce' ) ),
						$min_files
					),
				);
			}
	...
	function validate_on_checkout( $posted ) {
		...
		for ( $i = 1; $i <= $total_number; $i++ ) {
			if (
				...
				// validate min/max files
				$validated_min_max = $this->validate_files_min_max( $i, $_SESSION[ 'alg_checkout_files_upload_' . $i ] );
				if ( $validated_min_max !== true ) {
					wc_add_notice( $validated_min_max['message'], 'error' );
					continue;
				}
	...
	public function get_the_form( $file_uploader, $files, $order_id = 0 ) {
		...
		$multiple = get_option( 'alg_checkout_files_upload_multiple_' . $file_uploader, 'no' ) === 'yes' ? true : false;
		...
		$button_html .= '<input type="button" ' .
			...
			'value="' . ( $multiple ?
				( get_option( 'alg_checkout_files_upload_label_button_multiple_' . $file_uploader ) > '' ?
					get_option( 'alg_checkout_files_upload_label_button_multiple_' . $file_uploader )
					: __( 'Choose Files', 'checkout-files-upload-woocommerce' ) )
				: ( get_option( 'alg_checkout_files_upload_label_button_single_' . $file_uploader ) > '' ?
					get_option( 'alg_checkout_files_upload_label_button_single_' . $file_uploader )
					: __( 'Choose File', 'checkout-files-upload-woocommerce' ) )
			) . '" ' .
			'style="' . ( $files && ! $multiple ? 'display: none;' : '' ). '" />';
